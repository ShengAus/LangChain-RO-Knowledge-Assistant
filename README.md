...
It serves a browser UI at `/`, exposes a `POST /ask` endpoint, retrieves relevant content from markdown files in `data/`, and returns an answer with source files. If an OpenAI-compatible API key is configured, the final answer is generated from retrieved context. Without an API key, the app returns a deterministic fallback answer from the matched local text.
...
- Without an API key, the app returns a fallback answer from the local notes.
...
9. The response includes the answer and the supporting source files.
